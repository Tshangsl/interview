...
afrigo非洲跨境电商平台app端开发
    ...
    使用到的技术栈：架构基于国云 使用的是hbuilderx编辑器在dev分支开发 h5端调试 和uni-app封装好的一些API 写CSS样式用的是SASS语法
    遇到的哪些问题以及如何解决:
        这个项目我实现的功能部分其实不是很复杂 
        很多东西都有写好的组件 我找到合适的组件去调用就好了
        1.
            1.首先就是评论页面的样式 uni.redirectTo
            带用户个人信息参数点击评论图标进评论页面
            最开始没有啥经验 弄了一堆输入框
            其实非常不人性化 然后我就copy了一下淘宝评论页的页面
            2.最上面 左边一个商品小图片 右面是商品名 订单详情什么的
            写了一点样式 展示了一下
            3.下面使用了一个国云里(其实就是uni-app里)的rate(评分)组件 实现 点星星表示商品满意度
            4.然后就是一个textarea 里面填写用户评论
            5.再下面就是上传评论图片这个功能
            ...
            uni.chooseImage
            uni.uploadFile
            那时候思路可能也不完善 我是去淘宝评论页面 切了个图
            就是为宝贝拍买家秀那个图 我把它切下来放到项目静态图片文件夹里了
            我是用了一个三元表达式 默认用户没有上传的时候 显示那张图
            用户有上传图片显示用户的图
            也能正常显示 传到后端数据库中
            但是问题还是挺多的 
            首先只能上传一张图片 
            其次用切下来的图片占位 文字是中文的 
            而这个项目是要在非洲运行的 主要语言有英语法语两种
            不同语种的用户使用 应该能切换到不同语种
            当然可以使用两种语言的图片 但是那就意味着要多上传两张图片
            非洲那边流量很贵 如果你打开这个app使用成本太高的话 
            用户根本就可能不愿意再次打开
            考虑到这个使用成本的问题 项目做了一系列优化
            ...
            优化：
                iconfont是用字体文件取代图片文件，来展示图标、特殊字体等元素
            afrigo封装了一些功能模块：
            1.filters:过滤器
            2.mixins:vue混入
