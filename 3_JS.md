...
7.什么是闭包(closure)(不在所在词法作用域执行作用),闭包的作用是什么
    MDN闭包概念：(不在所在词法作用域下执行作用的)
        一个函数对其周围环境(词法环境)的引用捆绑在一起(或者说函数被引用包围)这样的组合就是闭包(closures)
        闭包让你可以在一个内层函数中访问到其外层函数的作用链
        在JavaScript中每当创建一个函数 闭包就会在函数创建的同时被创建出来
    闭包：(不在所在词法作用域中执行作用的)
