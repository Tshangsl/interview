...
6.JS中实现继承的方法有哪些
    ...
    2.原型继承
    3.借用构造函数继承
    4.寄生组合式继承(combination inheritance)
7.什么是闭包,闭包的作用是什么
...
6.作用域和执行上下文的区别是什么？
7.BFC(块级格式化上下文规则)和IFC BFC是什么 有哪几种实现方式 分别适用于哪些场景
    1.页面必须自动包含突出的浮动元素
    2.所有块级盒子的左边界默认与包含块的左边界对齐
        四种实现(触发BFC规则)方式
            1.display属性设置为inline-block 或者是 table-cell 可以为内容创建类似块级的上下文
            2.float属性值不是none的元素
            3.绝对定位的元素
            4.overflow属性设置为不是visible属性值
...
35.怎么拿到缓存区代码
36.用class创建对象和用function构建函数对象有什么不同
37.axios的底层用了什么
38.新的fetch请求时怎么实行的