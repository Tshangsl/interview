...
2.get和post区别
    最直观的就是语义上的区别：
        ...
        post提交数据|参数无长度限制|数据放在http请求体formData|不会被浏览器主动缓存|参数不会被保存在浏览器中|产生两个TCP数据包)
...
6.localStorage(本地存储 会话结束时清除)和 
    sessionStorage(会话存储 手动清除)---解决了cookie存储空间不足问题 与cookie对比
    1.简介
    ...
    5.五种跨域方法 JSONP CORS Node中间件代理 nginx反向代理 postMessage(H5新增)
        1.JSONP(JSON with padding)--需要服务器端配合
            ...
            缺点：
                ...
                3.容易受到XSS(跨站脚本)攻击 
        2.CORS  Cross-origin resource sharing 跨域资源共享 (需要浏览器和后端同时支持)
