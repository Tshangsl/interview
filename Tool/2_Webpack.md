### webpack
> 定义
- 本质上 webpack是一个现代JS应用程序的静态模块打包器(module bundler) 当webpack处理应用程序时 它会递归地构建一个依赖关系图(dependency graph) 其中包含应用程序需要的每个模块 然后将这些模块打包成一个或多个bundle
> 核心概念
1. 入口 entry
2. 输出 output
3. loader
4. 插件 plugins
> 意义
- 随着开发复杂度的提高 为了更快更好的开发程序 会大量使用到模块化开发 以及一些优秀的预处理器如less 但这些模块化工具 浏览器不能够直接识别 需要将开发的代码转为大部分浏览器能识别的代码 需要一个工具 将模块进行处理 这个处理的过程就是打包 webpack最简单的解释就是前段模块化打包工具
### Tree Shaking
- 引入一个模块时 不引入这个模块的所有代码 只引入需要的代码 需要借助Webpack中自带的Tree Shaking功能帮忙实现
- 官方:Tree-shaking本质是消除无用的JS代码 无用代码消除在广泛存在于传统的编程语言编译器 编译器可以判断出某些代码根本不影响输出 然后消除这些代码 这个称为DCE(dead code elimination)
- 在Webpack项目中有一个入口文件 相对于一棵树的主干 入口文件有很多依赖的模块 相当于树枝 实际情况中 虽然依赖了某个模块 但其实只是用其中的某些功能 通过Tree-Shaking 将没有使用的模块摇掉 达到删除无用代码的目的
