...
17.ES6中对 字符串 数值 数组 对象 函数的扩展
    ...
    2.数值：
        ...
        3.Number.isInteger()
            ...
            实现原理，用向下取整进行判断
    3.数组：(构造器新增方法Array.from() /原型新增方法keys values entries)
            构造器上新增的方法
                1.Array.of() 
...
28.严格模式
    (ES6 的模块化自动采用严格模式，不管你有没有在模块头部加上"use strict";)
    严格模式限制:
        变量必须声明后再使用
        函数的参数不能有同名属性，否则报错
        不能使用with语句
