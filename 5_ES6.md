...
6.模块化开发的概念 COMMONJS/ES6模块化方案的不同 
    模块化开发：
    ...
    ES6模块化(import export 和export default)
        ...
        /export default 仅能有一个/不需 因为只可能唯一对应)
            ...
            5.使用export default命令，为模块指定默认输出，不需要知道所要加载模块的变量名 
                ...
                所以它后面不能跟变量声明语句。
            6.export default命令用于指定模块的默认输出
                显然，一个模块只能有一个默认输出，因此export default命令只能使用一次。所以，import命令后面才不用加大括号，因为只可能唯一对应export default命令。
    CommonJS(运行时加载/require命令 同步加载/对模块浅拷贝/默认不是严格模式)和
    ES6(编译时输出/import命令 异步加载/对模块引用 即只读/默认是严格模式)
    (共同点 对象内部属性值进行改变/import export最终编译成require exportes执行)
        区别：
