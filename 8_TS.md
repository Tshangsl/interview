...
2. null和undefined是其他类型(包括void)的子类型 可以赋值给其他类型(如数字类型)
    ...
    - strictNullChecks参数用于新的严格空检查模式 在严格空检查模式下 null和undefined值都不属于任何一个类型 它们只能复制给自己这种类型或any
3. never和void的区别
...
15. 可选链运算符的使用
    ...
    3. 可选连运算符处于stage3阶段 使用@babel/plugin-proposal-optional-chaining插件可以提前使用 TS3.7版本正式支持使用 以前的版本会报错
