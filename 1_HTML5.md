...
6.H5中websocket(H5的一种新协议 防范被用做即时通讯 以代替轮询)
    (服务器和浏览器通过HTTP协议进行一个握手的动作 然后单独建立一条TCP的通信通道进行数据的传送)
    定义：
...
7.HTML文档中head标签里有哪些标签，说说他们的作用
    ...
    4. <meta> => 可提供有关页面的元信息（meta-information）,比如针对搜索引擎和更新频度的描述和关键词。
    5. <base> => 为页面上的所有链接规定默认地址或默认目标。
